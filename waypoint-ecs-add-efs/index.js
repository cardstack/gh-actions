...
async function main() {
  try {
    ...
    console.log("Adding volume to task definition…");
...
function addVolume(cluster, service, volumeName, fileSystemId, accessPointId, containerPath) {
  const taskDefinition = JSON.parse(
    execute(`aws ecs describe-task-definition --task-definition ${service.taskDefinition}`)
  );

  if (taskDefinitionHasVolume(taskDefinition.taskDefinition, volumeName, fileSystemId, accessPointId)) {
